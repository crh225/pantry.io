...
export const PantryList: React.FC = () => {
  ...
  const handleSave = (updated: PantryItemType) => { dispatch(updateItem(updated)); setEditing(null); };
  if (items.length === 0) return <div className="empty-pantry">Your pantry is empty. Add some items to get started!</div>;
  return (
    <div className="pantry-list">
      <div className="pantry-list-header">
        <span className="pantry-summary">{items.length} item{items.length !== 1 ? 's' : ''}</span>
        {items.length > 10 && <input className="pantry-search" placeholder="Search items..." value={filter} onChange={e => setFilter(e.target.value)} />}
      </div>
      <div className="pantry-columns">
        {sections.map(s => <PantrySection key={s.key} label={s.label} items={grouped[s.key] || []} onRemove={handleRemove} onEdit={handleEdit} />)}
      </div>
      {editing && <EditItemModal item={editing} onSave={handleSave} onClose={() => setEditing(null)} />}
    </div>
  );
};
